...
function Orders() {
        ...
        <Table.ScrollContainer>
          <Table.Content aria-label={title}>
            ...
            <Table.Body
              renderEmptyState={() => (
                <EmptyState
                  className={`flex table-empty-${type} h-full w-full flex-col items-center justify-center gap-4 text-center`}
                >
              ...
              {orders.map((order: OrderResponse) => {
                ...
                return (
                  <Fragment key={order.id}>
                    ...
                    {isExpanded && (
                      <OrderItems order={order} colNum={11} tableType={type} />
                    )}
                  </Fragment>
                );
              })}
  ...
  return (
    <div className="-mt-12 flex flex-col gap-4">
      ...
      <nav className="sticky top-0 z-40 bg-(--color-header-2) w-full border-b border-separator bg-background/70 backdrop-blur-lg">
